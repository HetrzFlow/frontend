...
export function formatAmountHuman(
  ...
  // Helper to determine if negative sign should be shown
  // Avoid displaying "-0.00" when the formatted result rounds to zero
  const shouldShowNegative = (formatted: string) => isNegative && parseFloat(formatted) !== 0;

  if (absN >= 1_000_000_000) {
    const formatted = (absN / 1_000_000_000).toFixed(displayDecimals);
    return `${shouldShowNegative(formatted) ? "-" : ""}${sign}${formatted}b`;
  }

  if (absN >= 1_000_000) {
    const formatted = (absN / 1_000_000).toFixed(displayDecimals);
    return `${shouldShowNegative(formatted) ? "-" : ""}${sign}${formatted}m`;
  }

  if (absN >= 1000) {
    const formatted = (absN / 1_000).toFixed(displayDecimals);
    return `${shouldShowNegative(formatted) ? "-" : ""}${sign}${formatted}k`;
  }

  const formatted = absN.toFixed(displayDecimals);
  return `${shouldShowNegative(formatted) ? "-" : ""}${sign}${formatted}`;
}
