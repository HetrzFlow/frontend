...
export function applySwapImpactWithCap(
  ...
  const price = priceImpactDeltaUsd > 0 ? prices.maxPrice : prices.minPrice;
...
export function getMaxPositionImpactFactors(marketInfo: {
  maxPositionImpactFactorPositive: bigint;
  maxPositionImpactFactorNegative: bigint;
}) {
...
export function getProportionalPendingImpactValues({
  ...
  const proportionalPendingImpactDeltaUsd = convertToUsd(
    ...
    indexToken.decimals,
    proportionalPendingImpactDeltaAmount > 0 ? indexTokenPrices.minPrice : indexTokenPrices.maxPrice
  )!;
...
export function getPriceImpactForSwap(
  marketInfo: MarketInfo,
  tokenAPrices: TokenPrices,
  tokenBPrices: TokenPrices,
  tokenA: TokenData,
  tokenB: TokenData,
  ...
  const [longTokenPrices, shortTokenPrices] =
    tokenAPoolType === "long" ? [tokenAPrices, tokenBPrices] : [tokenBPrices, tokenAPrices];
...
export function getNextPoolAmountsParams(p: {
  ...
  const longPrice = getMidPrice(longTokenPrices);
  const shortPrice = getMidPrice(shortTokenPrices);
