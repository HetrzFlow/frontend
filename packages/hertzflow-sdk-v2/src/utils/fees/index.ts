...
export function getDepositFee(marketInfo: MarketInfo, depositAmount: bigint, balanceWasImproved: boolean) {
  const factor = balanceWasImproved
    ? marketInfo.depositFeeFactorForBalanceWasImproved
    : marketInfo.depositFeeFactorForBalanceWasNotImproved;

  return applyFactor(depositAmount, factor);
}
