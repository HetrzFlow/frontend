...
const addressCache = new LRUCache<`0x${string}`>(10_000);

export function hashAddress(address: `0x${string}`) {
  if (addressCache.has(address)) {
    return addressCache.get(address)!;
  }

  const hash = encodeAbiParameters([{ type: "address" }], [address]);
  addressCache.set(address, hash);
...
export function keccakString(string: string) {
  return keccak256(toBytes(string));
}
