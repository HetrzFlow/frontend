...
export class Multicall {
  ...
  static async getInstance(sdk: HertzFlowSDK) {
  ...
  async call(request: MulticallRequestConfig<any>, maxTimeout: number) {
    ...
    const result = await Promise.race([
      ...
      sleep(maxTimeout, timeoutController.signal).then(() =>
        Promise.reject(new Error(this.sdk.config.settings?.ignoreTimeoutError ? "" : "multicall timeout"))
      ),
    ])
      .then((response) => {
    ...
    // decision reason: avoid noisy console errors in frontend; callers should inspect
    // MulticallResult.success / errors themselves instead of relying on global logs.
    return result;
  }
}
