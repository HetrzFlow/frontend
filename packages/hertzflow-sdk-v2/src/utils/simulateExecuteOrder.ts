 ...
 */
export async function simulateExecuteOrder(sdk: HertzFlowSDK, p: SimulateExecuteParams) {
...
function getSimulationPrices(
  ...
  for (const address of tokenAddresses) {
    ...
    if (!token || !prices[address] || primaryTokens.includes(convertedAddress)) {
      continue;
    }
    ...
    const currentPrice = {
      min: convertToContractPrice(prices[address].minPrice, token.decimals),
      max: convertToContractPrice(prices[address].maxPrice, token.decimals),
    };
