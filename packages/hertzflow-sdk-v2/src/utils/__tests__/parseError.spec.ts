...
describe("parseError", () => {
  ...
  describe("transaction errors", () => {
    ...
    it("should handle a wallet rejection with object details", () => {
      const error: ErrorLike = {
        info: {
          error: {
            code: 4001,
    ...
    it("should identify a wallet rejection by its standard code", () => {
    ...
    it("should handle RPC OutOfFunds error", () => {
      const error: ErrorLike = {
        info: {
          error: {
            code: -32003,
      ...
      expect(result).toEqual(
        expect.objectContaining({
          errorMessage: "EVM error: OutOfFunds",
          shortMessage: "EVM error: OutOfFunds",
          txErrorType: TxErrorType.NotEnoughFunds,
          isUserError: true,
          isUserRejectedError: false,
          errorGroup: "Txn Error: NOT_ENOUGH_FUNDS",
    ...
    it("should expose an unknown info error as a display message", () => {
      const error: ErrorLike = {
        info: {
          error: {
    ...
    it("should prioritize an inner info error when classifying", () => {
      const error: ErrorLike = {
    ...
    it("should handle OutOfFunds in a viem cause chain", () => {
      ...
      const transactionError = Object.assign(new Error("Transaction creation failed."), {
      ...
      expect(result).toEqual(
        expect.objectContaining({
          errorMessage: "EVM error: OutOfFunds",
          shortMessage: "Transaction creation failed.",
          rpcMessage: "EVM error: OutOfFunds",
          txErrorType: TxErrorType.NotEnoughFunds,
          isUserError: true,
          isUserRejectedError: false,
        })
    ...
    it("should prioritize insufficient funds details over a generic RPC code", () => {
      ...
      expect(result).toEqual(
        expect.objectContaining({
          errorMessage: "err: insufficient funds for gas * price + value",
          rpcMessage: "err: insufficient funds for gas * price + value",
          txErrorType: TxErrorType.NotEnoughFunds,
          isUserError: true,
        })
      );
    ...
    it("should handle contract errors with data", () => {
      ...
      expect(result).toEqual(
        expect.objectContaining({
          ...
          shortMessage: "OrderNotFulfillableAtLimit",
    ...
    it("should decode a standard Solidity revert reason from shortMessage", () => {
      const revertData =
        "";
      const error: ErrorLike = {
        name: "RpcRequestError",
        details: `execution reverted: ReferralStorage: public access disabled: ${revertData}`,
        shortMessage: revertData,
      };

      expect(parseError(error)).toEqual(
        expect.objectContaining({
          reason: "ReferralStorage: public access disabled",
          rpcMessage: undefined,
          shortMessage: "ReferralStorage: public access disabled",
        })
      );
    });
