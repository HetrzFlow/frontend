...
describe("getPositionKey", () => {
  it("returns key in expected format", () => {
    expect(getPositionKey("0xabc", "0xmarket", "0xcollateral", true)).toBe("0xabc:0xmarket:0xcollateral:true:false");
  });

  it("includes isZFP in key when present", () => {
    expect(getPositionKey("0xabc", "0xmarket", "0xcollateral", true, true)).toBe(
      "0xabc:0xmarket:0xcollateral:true:true"
    );
  });
});
...
describe("parsePositionKey", () => {
  it("parses position key string into an object", () => {
    const result = parsePositionKey("0xabc:0xmarket:0xcollateral:false:true");
  ...
  it("defaults isZFP to false for legacy keys", () => {
...
describe("getLiquidationPrice", () => {
  ...
  it("returns undefined if sizeInUsd <= 0 or sizeInTokens <= 0", () => {
    ...
    expect(
      getLiquidationPrice({
        indexTokenPrices: { minPrice: expandDecimals(1, USD_DECIMALS), maxPrice: expandDecimals(1, USD_DECIMALS) },
        sizeInUsd: 0n,
        sizeInTokens: 100n,
