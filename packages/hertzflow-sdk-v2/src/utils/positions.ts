...
export function getPositionKey(
  ...
  return `${account}:${marketAddress}:${collateralAddress}:${isLong}:${isZFP ?? false}`;
}

export function parsePositionKey(positionKey: string) {
  const [account, marketAddress, collateralAddress, isLong, isZFP] = positionKey.split(":");
...
export function getPositionPnlUsd(p: {
  ...
  const { marketInfo, sizeInUsd, sizeInTokens, markPrice, isLong, prices, tokensData } = p;
  ...
  const poolPnl = getMarketPnl(marketInfo, prices[marketInfo.indexTokenAddress], isLong, true);
...
export function getLiquidationPrice(p: {
  ...
  if (useMaxPriceImpact) {
    ...
    const pendingImpactUsd = convertToUsd(
      ...
      pendingImpactAmount > 0 ? indexTokenPrices.minPrice : indexTokenPrices.maxPrice
    )!;
...
export function getNetPriceImpactDeltaUsdForDecrease({
  ...
  marketInfo: PositionImpactMarketInfo;
  indexTokenPrices: TokenPrices;
  sizeInUsd: bigint;
  pendingImpactAmount: bigint;
  sizeDeltaUsd: bigint;
  ...
  const { proportionalPendingImpactDeltaUsd } = getProportionalPendingImpactValues({
    indexTokenPrices: indexTokenPrices,
    sizeInUsd,
    sizeDeltaUsd,
    pendingImpactAmount,
  ...
  totalImpactDeltaUsd = capPositionImpactUsdByMaxImpactPool(marketInfo, indexTokenPrices, totalImpactDeltaUsd);
