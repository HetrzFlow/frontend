...
import { TokenData, TokenPrices, TokensData, TokensRatio } from "types/tokens";
import {
  ExternalSwapQuote,
...
type IncreasePositionParams = {
  ...
  initialCollateralTokenPrices: TokenPrices;
  initialCollateralToken: TokenData;
  collateralTokenPrices: TokenPrices;
...
export function getIncreasePositionAmounts(p: IncreasePositionParams): IncreasePositionAmounts {
  ...
  const initialCollateralTokenPrice = initialCollateralTokenPrices.minPrice;
  const collateralTokenPrice = collateralTokenPrices.minPrice;
  ...
  const acceptablePriceInfo = getAcceptablePriceInfo({
    marketInfo,
    indexTokenPrices: prices[marketInfo.indexTokenAddress],
...
export function getIncreasePositionPrices({
  ...
  triggerPrice?: bigint;
  indexTokenPrice?: bigint;
  initialCollateralTokenPrice: bigint;
  ...
  if (triggerPrice !== undefined && triggerPrice > 0 && limitOrderType !== undefined) {
    ...
    collateralPrice = getIsEquivalentTokens(indexToken, collateralToken) ? triggerPrice : collateralTokenPrice;

    triggerThresholdType = getOrderThresholdType(limitOrderType, isLong);
  } else {
    indexPrice = indexTokenPrice!; //getMarkPrice({ prices: indexToken.prices, isIncrease: true, isLong });
    initialCollateralPrice = initialCollateralTokenPrice;
