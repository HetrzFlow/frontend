...
export function getDecreasePositionAmounts(p: {
  ...
  const indexTokenPrices = prices[indexToken.address];
  const collateralTokenPrices = prices[collateralToken.address];
  ...
  const pnlTokenPrices = prices[pnlToken.address];

  values.decreaseSwapType = getDecreaseSwapType(pnlToken, collateralToken, receiveToken);

  const markPrice = getMarkPrice({ prices: indexTokenPrices, isIncrease: false, isLong });
  ...
  if (orderType) {
    ...
    values.collateralPrice = collateralTokenPrice;
  }
  ...
  if (values.isFullClose) {
    ...
    values.collateralDeltaUsd =
      /**
       * leverageWithoutPnl may be zero if sizeInUsd is defaulted to 0n when
       * position not ready yet.
       */
...
export function estimateCollateralCost(
  baseUsd: bigint,
  collateralToken: TokenData,
  collateralMarkPrice: bigint,
  collateralPrice: bigint
) {
  const amount = convertToTokenAmount(baseUsd, collateralToken.decimals, collateralMarkPrice)!;
