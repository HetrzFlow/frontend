...
export const createFindSwapPath = (params: {
  ...
  const marketEdgeLiquidityGetter = createMarketEdgeLiquidityGetter(marketsInfoData, prices, tokensData);
  const naiveEstimator = createNaiveSwapEstimator(marketsInfoData, tokensData, prices, isAtomicSwap);
  const naiveNetworkEstimator = gasEstimationParams
    ? createNaiveNetworkEstimator({
        nativeTokenPrices: prices[wrappedToken.address],
    ...
    : undefined;
  const estimator = createSwapEstimator(marketsInfoData, tokensData, prices, isAtomicSwap);
