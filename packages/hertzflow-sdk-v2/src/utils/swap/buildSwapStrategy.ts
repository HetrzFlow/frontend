...
export function buildSwapStrategy({
  ...
  const tokenInPrices = prices[tokenIn.address];
  const tokenOutPrices = prices[tokenOut.address];
  const priceIn = tokenInPrices.minPrice;
  ...
  const defaultSwapStrategy: SwapStrategyForSwapOrders = {
    ...
    amountIn,
    amountOut: convertToTokenAmount(usdIn, tokenOut.decimals, tokenOutPrices.maxPrice)!,
    usdIn,
    usdOut: usdIn,
    priceIn,
    priceOut: tokenOutPrices.maxPrice,
    feesUsd: 0n,
  };
...
export function buildReverseSwapStrategy({
  ...
  const tokenInPrices = prices[tokenIn.address];
  const tokenOutPrices = prices[tokenOut.address];
  const priceIn = getMidPrice(tokenInPrices);
  const priceOut = getMidPrice(tokenOutPrices);

  const preferredUsdOut = convertToUsd(amountOut, tokenOut.decimals, getMidPrice(tokenOutPrices))!;
  const approximateAmountIn = convertToTokenAmount(preferredUsdOut, tokenIn.decimals, getMidPrice(tokenInPrices))!;
  ...
  if (approximateSwapPathStats) {
    ...
    const adjustedUsdIn =
      approximateSwapPathStats.usdOut > 0
        ...
        : 0n;
    const adjustedAmountIn = convertToTokenAmount(adjustedUsdIn, tokenIn.decimals, getMidPrice(tokenInPrices))!;
