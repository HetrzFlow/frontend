...
export function getSwapCapacityUsd(marketInfo: MarketInfo, isLong: boolean, tokenPrices: TokenPrices) {
  ...
  const capacityUsd = convertToUsd(capacityAmount, token.decimals, getMidPrice(tokenPrices))!;
...
export function getSwapStats(p: {
  ...
  const indexTokenPrices = prices[marketInfo.indexTokenAddress];
  const tokenInPrices = prices[tokenInAddress];
  const tokenOutPrices = prices[tokenOutAddress];
  ...
  const priceOut = tokenOutPrices.minPrice;
...
export function getMaxSwapPathLiquidity(p: {
  ...
  const { marketsInfoData, swapPath, initialCollateralAddress, prices, tokensData } = p;
  ...
  for (const marketAddress of swapPath) {
    ...
    const liquidity = getAvailableUsdLiquidityForCollateral(marketInfo, isTokenOutLong, prices, tokensData);
