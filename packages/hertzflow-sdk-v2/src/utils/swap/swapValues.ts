...
import type { TokenData, TokenPrices, TokensData, TokensRatio } from "types/tokens";
...
function getSwapAmountsByFromValueDefault(p: {
  ...
  const priceOut = tokenOutPrices.maxPrice;
  ...
  if (getIsStake(tokenIn, tokenOut) || getIsUnstake(tokenIn, tokenOut)) {
    return getPlainSwapAmountsByFromToken(tokenIn, tokenOut, amountIn, tokenInPrices, tokenOutPrices);
  }
...
function getSwapAmountsByToValueDefault(p: {
  ...
  if (getIsStake(tokenIn, tokenOut) || getIsUnstake(tokenIn, tokenOut)) {
    return getPlainSwapAmountsByToToken(tokenIn, tokenOut, amountOut, tokenInPrices, tokenOutPrices);
  }
...
function getPlainSwapAmountsByFromToken(
  ...
  const usdIn = convertToUsd(amountIn, tokenIn.decimals, tokenInPrices.minPrice)!;
  const usdOut = usdIn;
  const amountOut = convertToTokenAmount(usdOut, tokenOut.decimals, tokenOutPrices.maxPrice)!;
  const priceIn = tokenInPrices.minPrice;
  const priceOut = tokenOutPrices.maxPrice;
