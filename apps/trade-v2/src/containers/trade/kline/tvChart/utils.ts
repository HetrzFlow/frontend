...
export const inSamePeriod = (
  ...
  if (unit === 'M') {
    const date1 = new Date(finalTs1 * 1000);
    const date2 = new Date(finalTs2 * 1000);
    const monthIndex1 = date1.getFullYear() * 12 + date1.getMonth();
    const monthIndex2 = date2.getFullYear() * 12 + date2.getMonth();
    return Math.floor(monthIndex1 / interval) === Math.floor(monthIndex2 / interval);
  }
