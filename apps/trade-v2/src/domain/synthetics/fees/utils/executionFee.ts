...
export function estimateExecutionGasPrice(p: {
  ...
  const gasPrice = rawGasPrice + maxPriorityFeePerGas;
...
export function getExecutionFeeBufferBps(
  chainId: number,
  settledBufferBps: number | undefined,
) {
  return BigInt(
    settledBufferBps ??
      getExecutionFeeConfig(chainId as ContractsChainId)?.defaultBufferBps ??
      0,
  );
}
...
export function getMaxPriorityFeePerGas(
  chainId: number,
  onChainMaxPriorityFeePerGas: bigint | undefined | null,
) {
...
export function getMinimumExecutionFeeBufferBps(p: {
  ...
  const estimatedGasPriceWithBuffer =
    estimatedExecutionFee / gasLimit - premium;

  const baseGasPrice =
    (estimatedGasPriceWithBuffer * BASIS_POINTS_DIVISOR_BIGINT) /
    (BASIS_POINTS_DIVISOR_BIGINT + currentBufferBps);
  ...
  const bufferBps =
    (targetGasPrice * BASIS_POINTS_DIVISOR_BIGINT) / baseGasPrice -
    BASIS_POINTS_DIVISOR_BIGINT;

  // Add extra 5% for safety
  const requiredBufferBps =
    bufferBps + (BASIS_POINTS_DIVISOR_BIGINT / 100n) * 5n;
